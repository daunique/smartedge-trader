...
import { Search, Filter, Download, TrendingUp, TrendingDown, CheckCircle, XCircle } from 'lucide-react'
...
export default function History() {
  ...
  const [filter, setFilter] = useState('ALL') // ALL | TP | SL | CRYPTO | FOREX
  const [sort, setSort] = useState('date') // date | pnl | rr
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="grid grid-cols-12 gap-2 px-4 py-2.5 border-b border-bg-border bg-bg-elevated/50">
          {['Symbol', 'Dir', 'Status', 'P&L', 'RR', 'ML', 'Duration', 'Date'].map((h, i) => (
            <div key={h} className={clsx(
              'font-body text-xs text-text-muted uppercase tracking-wider',
              i === 0 ? 'col-span-2' : i === 3 ? 'col-span-2' : i === 7 ? 'col-span-2 hidden md:block' : 'col-span-1',
              [5, 6].includes(i) ? 'hidden lg:block' : '',
            )}>{h}</div>
          ))}
        </div>
          ...
          {filtered.map(trade => (
            <div key={trade.id} className="grid grid-cols-12 gap-2 px-4 py-3 hover:bg-bg-elevated/40 transition-colors items-center">
              <div className="col-span-2">
                ...
                <div className="font-body text-xs text-text-muted">{trade.market}</div>
              </div>
              <div className="col-span-1">
                <span className={clsx('font-body text-xs font-semibold', trade.direction === 'LONG' ? 'text-accent-green' : 'text-accent-red')}>
              ...
              <div className="col-span-2">
                <span className={clsx('font-display text-sm font-bold', trade.pnl >= 0 ? 'text-accent-green' : 'text-accent-red')}>
